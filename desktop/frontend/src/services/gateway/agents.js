...
function agentPayload(input, options = {}) {
  const body = {
    ...
    network_proxy: {
      ...
      https_proxy: String(input.httpsProxy || '').trim(),
      no_proxy: String(input.noProxy || '').trim(),
    },
...
function normalizeAgent(agent) {
  return {
    ...
    httpProxy: agent.network_proxy?.http_proxy || '',
    httpsProxy: agent.network_proxy?.https_proxy || '',
    noProxy: agent.network_proxy?.no_proxy || '',
