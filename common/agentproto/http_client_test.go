...
func TestHTTPClientStreamHandlesLargeSSELine(t *testing.T) {
	...
	if event.Type != StreamEventSessionUpdate || event.Update == nil || event.Update.Content == nil || event.Update.Content.Text != largeOutput {
		t.Fatalf("event = %+v", event)
	}
}

func TestHTTPClientPreservesErrorCode(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
