...
func TestGatewayConversationACPTransportE2E(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping e2e process test in short mode")
	}
...
`, gatewayPort, slashPath(filepath.Join(tmp, "gateway.sqlite")), gatewayPort, token, slashPath(clawBin), slashPath(filepath.Join(tmp, "claw-configs")), clawPort, clawPort))
	...
	doJSON(t, http.MethodPost, gatewayURL+"/v1/conversations", map[string]any{
		"agent_id": "agent_acp_e2e",
		"title":    "ACP E2E Conversation",
	}, http.StatusCreated, &conversation)
	if conversation.ID == "" || conversation.SessionID == "" {
		t.Fatalf("conversation missing ids: %+v", conversation)
	}
	...
	doJSON(t, http.MethodPost, gatewayURL+"/v1/conversations/"+conversation.ID+"/messages", map[string]any{
		"prompt":     "hello acp e2e",
		"request_id": "req_acp_e2e",
	}, http.StatusOK, &chatResp)
	if chatResp.Output != "fake agent response: hello acp e2e" {
		t.Fatalf("output = %q", chatResp.Output)
	}
	...
	doJSON(t, http.MethodGet, gatewayURL+"/v1/agent-instances", nil, http.StatusOK, &instances)
	if len(instances.Instances) != 1 {
		t.Fatalf("instances = %+v, want one acp instance", instances.Instances)
	}
	if instances.Instances[0].Transport != "acp" || !strings.HasPrefix(instances.Instances[0].BaseURL, "acp://") {
		t.Fatalf("instance = %+v, want acp transport", instances.Instances[0])
	}
	...
	if messages.Messages[1].Role != "assistant" || messages.Messages[1].Content != "fake agent response: hello acp e2e" {
		t.Fatalf("assistant message = %+v", messages.Messages[1])
	}
}
