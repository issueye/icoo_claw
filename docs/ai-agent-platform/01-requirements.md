...
- 三服务启动配置使用 TOML 文件，通过 `--config` 指定。
...
- Claw 在运行前从 Session Store 加载历史。
- Claw 在运行结束后读取 runtime session snapshot 并保存到 Session Store。
- 同 session 并发冲突在 MVP 中返回 `409 session_busy`。
- fake runner 可用于本地端到端测试，真实 runner 使用 SDK model provider。

## Session Store 能力边界

Session Store 使用 GORM + no-cgo SQLite 作为 MVP 唯一持久化方案。
...
不再把额外兼容协议作为 MVP 依赖。若后续需要外部协议兼容能力，应作为 P2 独立兼容层设计。
...
- 支持 TOML 配置：HTTP 地址、Session Store URL、内部 token、runner mode。
- 支持从 Session Store 加载历史并保存 snapshot。
...
- 使用 GORM + `github.com/glebarez/sqlite` 保存会话数据。
- 提供 HTTP API 给 Gateway 和 Claw 使用。
- 提供 revision 字段保护 snapshot 覆盖。
- 支持端到端恢复会话历史。
