...
本文档集基于已抽取到 `server/claw/pkg/agent_sdk/sdk` 的 agentsdk-go 源码模块，以及当前 `server/claw`、`server/gateway`、`server/session_store` 三个服务的 MVP 与后续演进设计。
...
- [session-store-gorm-models.md](session-store-gorm-models.md): Session Store GORM + SQLite(no cgo) 明细模型设计。
...
- `session_store` 使用 GORM + `github.com/glebarez/sqlite`，以 no-cgo SQLite 保存会话、消息、run 与 run event。
